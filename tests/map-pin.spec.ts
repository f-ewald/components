...
test.describe("map-pin", () => {
  ...
  test("`ring-opacity` is configurable and drives the rendered ring's translucency", async ({ page }) => {
    ...
    await expect(page.locator("#pin-ring-opaque path")).toHaveAttribute("stroke", "rgb(255 255 255 / 1)");
    await expect(page.locator("#pin-ring-default-opacity path")).toHaveAttribute("stroke", "rgb(255 255 255 / 0.6)");
    await expect(page.locator("#pin-ring-faint path")).toHaveAttribute("stroke", "rgb(255 255 255 / 0.25)");
  });
  ...
  test("preserves map colors and geometry while removing highlight motion", async ({ page }) => {
    ...
    await expect(pin.locator("path")).toHaveAttribute("stroke", "rgb(255 255 255 / 0.6)");
